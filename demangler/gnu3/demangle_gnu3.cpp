...
static bool ReplaceAll(string& text, const string& needle, const string& replacement)
{
	bool changed = false;
	size_t pos;
	while ((pos = text.find(needle)) != string::npos)
	{
...
static bool ReplaceForwardRefInName(DemangledTypeNode& type, const string& placeholder, const string& replacement);


static bool ReplaceForwardRefInNamePart(DemangledNamePart& part, const string& placeholder, const string& replacement)
{
	bool resolved = false;
	string base = part.GetBase();
	if (ReplaceAll(base, placeholder, replacement))
	{
		part.SetBase(std::move(base));
		resolved = true;
	}
	for (auto& arg: part.GetMutableTemplateArguments())
	{
		if (arg.type)
			resolved |= ReplaceForwardRefInName(*arg.type, placeholder, replacement);
	}
	return resolved;
}
...
{
	bool resolved = false;
	for (auto& part: type.GetMutableName())
		resolved |= ReplaceForwardRefInNamePart(part, placeholder, replacement);
	return resolved;
}


void DemangleGNU3::ResolveForwardTemplateRefs(DemangledTypeNode& type, const ParamList& args)
{
	...
	for (const auto& fr : m_pendingForwardRefs)
	{
		...
		string replacement = (fr.index < args.size() && args[fr.index].type) ? args[fr.index].type->GetString() : "auto";
		resolved |= ReplaceForwardRefInName(type, placeholder, replacement);
	}
...
DemangledTypeNode DemangleGNU3::DemangleUnqualifiedName()
{
	...
	case hash('U','l'): //Lambda
	{
		...
		for (int autoIdx = 0; autoIdx < 16; autoIdx++)
			m_templateSubstitute.push_back(DemangledTypeNode::CreateShared(CreateUnknownType("auto")));
		do
		{
			DemangledTypeNode param = DemangleType();
			if (param.GetClass() == VoidTypeClass)
